...
from dataset_business import SoccerNetClips, SoccerNetClipsTesting #,SoccerNetClipsOld
from model import Model
from train_business import trainer, test, testSpotting
...
def main(args):
    ...
    if not args.test_only:
        dataset_Train = SoccerNetClips(path=args.SoccerNet_path, features=args.features, split=args.split_train, version=args.version, framerate=args.framerate, window_size=args.window_size, spread=args.spread)
        dataset_Valid = SoccerNetClips(path=args.SoccerNet_path, features=args.features, split=args.split_valid, version=args.version, framerate=args.framerate, window_size=args.window_size, spread=args.spread)
        dataset_Valid_metric  = SoccerNetClips(path=args.SoccerNet_path, features=args.features, split=args.split_valid, version=args.version, framerate=args.framerate, window_size=args.window_size, spread=args.spread)
    ...
    model = Model(weights=args.load_weights, input_size=args.feature_dim,
                  ...
                  framerate=args.framerate, pool=args.pool, drop=args.drop).cuda()
    logging.info(model)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--spread',   required=False, action='store_true',  help='Perform testing only' )
    ...
    parser.add_argument('--drop', required=False, type=float,   default=0.4,     help='Framerate of the drop' )
