...
class Aicity(torch.utils.data.Dataset):
    """
    aicity track3 video loader. Construct the video loader, then sample
    ...
    def __init__(self, cfg, mode, num_retries=10):
        ...
        assert mode in [
            ...
            "val",
            "test",
        ], "Split '{}' not supported for Aicity".format(mode)
        self.mode = mode
        self.cfg = cfg
        ...
        if self.use_multi_dataset:
            dataset_idx = self.cfg.MODEL.MULTI_DATASETS.index("aicity")
        ...
        logger.info("Constructing Aicity {}...".format(mode))
        self.val_skip = 1
    ...
    def _construct_loader(self):
        ...
        with pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                assert (
                    len(path_label.rsplit(self.cfg.DATA.PATH_LABEL_SEPARATOR,1))     #这里改了！
                    == 2
                )
                ...
                path, label = path_label.rsplit(
                    self.cfg.DATA.PATH_LABEL_SEPARATOR,1      #这里也改了！
                )
        ...
        ), "Failed to load Aicity split {} from {}".format(
            self._split_idx, path_to_file
        )
        logger.info(
            "Constructing Aicity dataloader (size: {}) from {}".format(
    ...
    def __getitem__(self, index):
        ...
        if self.use_multi_dataset:
            extra_data["dataset_name"] = "aicity"
        ...
        for i_try in range(self._num_retries):
            ...
            if self.aug:
                ...
                if self.cfg.AUG.NUM_SAMPLE > 1:
                    ...
                    if self.use_multi_dataset:
                        extra_data["dataset_name"] = ["aicity"] * self.cfg.AUG.NUM_SAMPLE
