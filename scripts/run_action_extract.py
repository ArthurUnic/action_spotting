...
parser.add_argument("--jobinter", default=5, type=int)
...
def main(args):
    ...
    if len(video_files) != -1:
        video_files =  video_files[args.jobid * args.jobinter:(args.jobid+1) * args.jobinter] 

    print('len(video_files)', len(video_files), video_files)
    ...
    # print('file', action_model.__file__)
    # print('file', )
    ...
    for video_file in tqdm(video_files):
        ...
        times = []
        pred_list = []
        # print('Save Feature on', os.path.join(args.out_dir, "%s.pth" % video_name))
        # rfeatures = torch.cat(features, dim=0)
        # torch.save({'features':rfeatures, 'times':pred_list}, os.path.join(args.out_dir, "%s.pth" % video_name))
        # assert False 
        for cur_iter, (frames, t0s, t1s) in tqdm(
            ...
            # [N, 600/num_class]
            preds, feas = action_model.inference(frames, extract_fea=True)
            feas = feas.cpu()
            for b in range(frames[0].shape[0]):
                t0, t1 = int(t0s[b].numpy()), int(t1s[b].numpy())
                pred_list.append((t0, t1))
